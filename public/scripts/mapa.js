const url = "/api/v2/ecircuit1/feeds/13bfed0eff764944a17783ac6d66b919";
const xhrbtn = document.querySelector("StatusVasilha");

xhrbtn.addEventListener("click", function(){
    const XHR = new HttpRequest();
    XHR.onreadystatechange = function(){
        if(XHR.readyState === 4 && XHR.status === 200){
        const dataSensor = JSON.parse(XHR.responseText);//transforma a str em um objeto
        display.innerText = dataSensor;
        }
    }
    XHR.open("GET", url);
    XHR.send();
